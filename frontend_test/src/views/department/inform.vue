    ...
    <b-modal id="modal-department-inform" :title="getTitle" @ok="onSubmit">
      <div>
        <b-form-group v-if="inputMode === 'update'" label="id" label-for="id" label-cols="3">
          <b-form-input id="name" v-model="department.id" disabled></b-form-input>
        </b-form-group>
        ...
        <b-form-group label="생성일" label-for="createAt" label-cols="3">
          <b-form-input id="createAt" v-model="department.createdAt" disabled></b-form-input>
        </b-form-group>
      </div>
    </b-modal>
  </div>
